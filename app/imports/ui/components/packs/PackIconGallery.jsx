...
const PacksIconGallery = ({ open, onClose, onSendImage }) => {
  ...
  const avImages = () => {
    const images = [];
    for (let i = 1; i < 329; i += 1) {
      images.push(`/images/packs/packs-${i.toString().padStart(3, '0')}.svg`);
    }
    return images;
  };
      ...
      <div className={paperStyle}>
        <Card sx={cardStyle}>
          <CardHeader
            ...
            action={<IconButtonEole onClick={onClose} icon={<ClearIcon />} />}
          />
          <CardContent>
            <Grid container>
              {avImages().map((src) => (
...
PacksIconGallery.propTypes = {
  open: PropTypes.bool.isRequired,
  onClose: PropTypes.func.isRequired,
  onSendImage: PropTypes.func.isRequired,
};

export default PacksIconGallery;
