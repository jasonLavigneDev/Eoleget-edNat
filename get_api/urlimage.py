# Modified version of urlimage-dcollien
# pip install urlimage-dcollien (https://pypi.org/project/urlimage-dcollien/)
import requests
...
def _get_meta_content(soup, attr, value):
    for meta in soup.find_all(name="meta"):
        ...
        if attr_val is not None:
            attr_val_list = [val.lower() for val in attr_val.split()]
...
def _get_link_href(soup, rel):
    ...
    for link in soup.find_all(name="link"):
        ...
        if rel_val is not None:
            rel_val_list = [val.lower() for val in rel_val]
...
def get_image_for_url(url, check_order=None):
    ...
    try:
        soup = _get_soup_from_url(url, timeout=10)
